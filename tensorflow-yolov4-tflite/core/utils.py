...
def load_weights(model, weights_file, model_name='yolov4', is_tiny=False):
    ...
    for i in range(layer_size):
        ...
        print(f"Expected conv shape: {conv_shape}")
        conv_weights = np.fromfile(wf, dtype=np.float32, count=np.product(conv_shape))
        print(f"Loaded conv_weights size: {conv_weights.size}")
        # tf shape (height, width, in_dim, out_dim)
        if conv_weights.size != np.product(conv_shape):
            raise ValueError(f"Mismatch in weight size for layer {conv_layer_name}: expected {np.product(conv_shape)}, got {conv_weights.size}")
